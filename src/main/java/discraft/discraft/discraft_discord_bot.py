...
async def on_message(message):
    if message.author != client.user and message.content not in comms:
        ...
        print(datetime.datetime.today().strftime("%d.%m.%Y %H:%M:%S") + " | 'Not command'-message received: " + user)
        with open('linked.json') as f:
            linked = json.load(f)
        with open('reg_codes.json') as f:
            reg_codes = json.load(f)
        with open('login_codes.json') as f:
        ...
        if not await check_reg(message, user):
            ...
            print(datetime.datetime.today().strftime("%d.%m.%Y %H:%M:%S") + " | User is already registered: " + user)
            return
        ...
        if not code.isdecimal():
            ...
            print(datetime.datetime.today().strftime("%d.%m.%Y %H:%M:%S") + " | Invalid code: " + user)
            return
        if code in reg_codes.values():
            ...
            with open('linked.json', 'w') as f:
                json.dump(linked, f)
            with open('reg_codes.json', 'w') as f:
                json.dump(reg_codes, f)
            with open('login_codes.json', 'w') as f:
...
async def unlink(message):
    ...
    print(datetime.datetime.today().strftime("%d.%m.%Y %H:%M:%S") + " | Unlink was successful: " + user)
...
async def stop(message):
    ...
    print(datetime.datetime.today().strftime("%d.%m.%Y %H:%M:%S") + " | The bot was stopped by " + str(message.author))
    exit(1)
